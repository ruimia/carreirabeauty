...
export default async function AssinaturasPage() {
  ...
  // MRR estimado
  const mrrEmpresas = (empresas ?? []).reduce((sum, e) => {
    return sum + (PLANOS_EMPRESA[e.plano as keyof typeof PLANOS_EMPRESA]?.preco ?? 0);
  }, 0);
  const mrrProfissionais = (profissionais ?? []).reduce((sum, p) => {
    return sum + (PLANOS_PROFISSIONAL[p.plano as keyof typeof PLANOS_PROFISSIONAL]?.preco ?? 0);
  }, 0);
  const mrr = mrrEmpresas + mrrProfissionais;

  const empresasPagas = (empresas ?? []).filter((e) => e.plano !== "gratis").length;
  const profissionaisPagos = (profissionais ?? []).filter((p) => p.plano !== "gratis").length;
  ...
  const statsByPlanoProfissional = PLANOS_PROF_KEYS.map((k) => ({
    ...
    preco: PLANOS_PROFISSIONAL[k].preco,
  }));
    ...
    <div className="space-y-6 py-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="rounded-xl p-4 bg-purple-50 text-purple-700 col-span-2 sm:col-span-1">
          ...
          <p className="text-sm mt-1 opacity-70">MRR estimado</p>
        </div>
        ...
        <div className="rounded-xl p-4 bg-gray-50 text-gray-600">
          ...
          <p className="text-sm mt-1 opacity-70">Total assinantes</p>
        </div>
      </div>
